...
for (let rawLine of raw.split("\n")) {
  ...
  if (f[0].length !== 40 || f.length < 8) continue;
  lines.push({ hash: f[0], shortHash: f[1], refs: f[5].split(",").map((s) => s.trim()).filter(Boolean), parents: f[6].split(" ").map((s) => s.trim()).filter(Boolean), subject: f[7] });
}
