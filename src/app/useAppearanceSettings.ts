...
function readAppearanceSettings(): AppearanceSettings {
  try {
    ...
    return {
      ...
      streamingFadeRange: boundedNumber(value.streamingFadeRange, defaultAppearance.streamingFadeRange, 2, 20),
...
export function useAppearanceSettings({ locale, onNotice, onError }: UseAppearanceSettingsOptions) {
  ...
  const appearanceStyle = useMemo(() => {
    ...
    const style: Record<string, string> = {
      ...
      "--stream-fade-range": `${appearance.streamingFadeRange}em`,
