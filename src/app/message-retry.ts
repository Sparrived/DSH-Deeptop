...
export function retryPromptSourceParts(content: unknown, locale: UiLocale = "zh"): RetryPromptSourcePart[] {
  ...
  for (const blockValue of content) {
    ...
    if (block.type !== "image") {
      throw new Error(t("message.retryUnsupportedContent", locale, { type: String(block.type ?? "unknown") }));
    }
    ...
    if (!mediaType || (!data && !attachmentId)) throw new Error(t("message.retryInvalidImage", locale));
    parts.push({
      type: "image",
      mediaType,
      ...(name ? { name } : {}),
