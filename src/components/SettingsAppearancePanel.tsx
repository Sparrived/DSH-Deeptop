      ...
      {section === "typography" && (
        <div className="settings-block">
          ...
          <div className="settings-preference-list">
            ...
            <label className="settings-preference-row"><span><strong>{t("appearance.streamingFadeDuration", locale)}</strong><small>{t("appearance.streamingFadeDurationHint", locale)}</small></span><span className="appearance-range-control"><input type="range" min="150" max="1500" step="50" value={appearance.streamingFadeDuration} onChange={(event) => onUpdate({ streamingFadeDuration: Number(event.target.value) })} /><output>{appearance.streamingFadeDuration}ms</output></span></label>
            <label className="settings-preference-row"><span><strong>{t("appearance.streamingFadeInk", locale)}</strong><small>{t("appearance.streamingFadeInkHint", locale)}</small></span><span className="appearance-range-control"><input type="range" min="5" max="100" step="5" value={Math.round(appearance.streamingFadeInk * 100)} onChange={(event) => onUpdate({ streamingFadeInk: Number(event.target.value) / 100 })} /><output>{Math.round(appearance.streamingFadeInk * 100)}%</output></span></label>
          </div>
        </div>
      )}
