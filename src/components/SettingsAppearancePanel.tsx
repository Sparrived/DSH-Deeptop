      ...
      {section === "typography" && (
        <div className="settings-block">
          ...
          <div className="settings-preference-list">
            ...
            <label className="settings-preference-row"><span><strong>{t("appearance.streamingFadeRange", locale)}</strong><small>{t("appearance.streamingFadeRangeHint", locale)}</small></span><span className="appearance-range-control"><input type="range" min="2" max="20" step="1" value={appearance.streamingFadeRange} onChange={(event) => onUpdate({ streamingFadeRange: Number(event.target.value) })} /><output>{appearance.streamingFadeRange}em</output></span></label>
