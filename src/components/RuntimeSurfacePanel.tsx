...
export function RuntimeSurfacePanel({
  ...
  return <>
    ...
    <div className="inspector-section"><span className="inspector-label">运行目录</span><div className="session-stats-inline">上下文 {sessionStats.contextTokensAvailable ? formatTokens(sessionStats.contextTokens) : "未提供"} · ↓ {formatTokens(sessionStats.inputTokens)} · ↑ {formatTokens(sessionStats.outputTokens)} · 缓存 {sessionStats.cacheHitRate ? String(sessionStats.cacheHitRate.toFixed(0)) + "%" : "未提供"}</div>{sessionStats.turns !== undefined && <div className="session-stats-inline">官方统计 {sessionStats.turns} 轮 · {sessionStats.steps ?? 0} 步</div>}<code>{status.runtimeDirectory || "未读取"}</code></div>
