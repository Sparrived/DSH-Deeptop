...
export function TokenUsageDashboard({ entries, sessionStats, active }: TokenUsageDashboardProps) {
  ...
  return <section className="token-dashboard" aria-label="Token 用量统计">
    ...
    <div className="token-dashboard-main">
      ...
      <div className="token-panel token-context-panel"><div className="token-panel-heading"><div><span>CONTEXT WINDOW</span><h3>上下文压力</h3></div><b>{sessionStats.contextLimit ? Math.round(contextPercent) + "%" : "—"}</b></div><div className="token-context-visual"><div className="token-context-ring" style={{ "--token-context-progress": contextPercent + "%" } as CSSProperties}><div><strong>{formatTokens(sessionStats.contextTokens)}</strong><span>当前上下文</span></div></div><div className="token-context-details"><span>已用 <b>{formatTokens(sessionStats.contextTokens)}</b></span><span>上限 <b>{sessionStats.contextLimit ? formatTokens(sessionStats.contextLimit) : "未提供"}</b></span><span>消息 <b>{sessionStats.messages}</b></span></div></div><div className="token-progress"><i style={{ width: contextPercent + "%" }} /></div></div>
    </div>
