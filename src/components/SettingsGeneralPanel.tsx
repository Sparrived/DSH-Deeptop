    ...
    <div className="settings-page">
      ...
      <div className="settings-block">
        ...
        <div className="settings-preference-list">
          ...
          <div className="settings-preference-row"><span><strong>会话侧栏</strong><small>{sidebarWidth}px · 拖动主界面分隔线调整</small></span><button onClick={onResetSidebar} disabled={sidebarWidth === 274}>恢复默认</button></div>
          <label className="settings-preference-row"><span><strong>主题</strong><small>桌面端界面显示方式</small></span><select value={themeMode} onChange={(event) => onThemeChange(event.target.value as ThemeMode)}><option value="system">跟随系统</option><option value="light">浅色</option><option value="dark">深色</option></select></label>
        </div>
      </div>
