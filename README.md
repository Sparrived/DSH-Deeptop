...
| **Agent interaction** | Watch tool calls, Todos, Workflows, Jobs and execution trajectories in one place, and answer approvals plus single-choice, multi-choice or free-form questions. Clicking a task expands its output: the bridge reads it through DSH's non-consuming projection (`ctx.jobs.peek`), refreshing while the task runs without taking what the model's `job_output` still owns. |
| **Models & providers** | Browse available providers and models, pick the model and reasoning effort per session, and declare Max reasoning for custom pi-ai models; a pi-ai model that declares no thinking levels can also be given Off/Low/Medium/High/Xhigh/Max straight from the composer, which records the declaration in that route's local settings. Credentials stay managed by the DSH API. |
...
- For local development: Node.js **22.21+** or **24+**, with `node` and `npm` on your `PATH`;
