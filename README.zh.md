...
| 模型与 Provider | 有边界地支持 | Provider/模型目录、每会话模型选择、思考程度、上下文窗口和输入模态元数据、Provider 发现及自定义连接设置；`llm-pi-ai` Provider 上未声明思考档位的模型可直接在发送框滑选 `Off/Low/Medium/High/Xhigh/Max`，首次提交把档位声明写进该路由的本地设置，设置里也能为已配置模型声明或关闭 `Max`。RC8 的 `current`/`routable` 是发送准入依据，`groups` 仅用于候选展示；Schema 驱动的 Provider 表单尚未完整实现。 |
...
4. 创建会话、选择模型并发送提示。回合运行中，发送按钮会显示本次投递方式（排队或插入），右侧箭头的上拉菜单可切换它；待处理消息也会各自标出是排队还是插入。输入 `@` 可通过 DSH RC8 File/Session Reference 选择文件、目录或会话；带空格的路径使用引号候选。
...
6. 未发现兼容的系统 Node.js 时，自动从 `nodejs.org` 下载该平台固定的 Node.js 22.21.1 发行包（Windows 为 ZIP，Linux/macOS 为 tar.gz），用编译期固定的 SHA-256 先校验归档、再校验解压出的可执行文件，然后仅把该可执行文件写入应用本地运行时缓存；不会修改系统 `PATH`，也不需要 npm。
...
10. 安装包资源只读；Profile、会话、日志和设置仍写入 `$DSH_HOME`，然后等待 Bridge 返回 `deeptop/1` 的 `ready` 帧。
...
Deeptop 通过系统 `PATH` 中的 Node.js 直接执行安装包内嵌的 DSH JavaScript；npm 不参与安装包运行时。系统未安装兼容版本时，Deeptop 会自动下载并校验官方 Node.js 22.21.1，只把可执行文件写入应用本地缓存，因此通常无需手动安装。自动配置失败或你希望使用自己的 Node.js 时，请安装 Node.js 22.21+ 或 24+，确认启动 Deeptop 的进程继承了正确的 `PATH`，然后重试。
