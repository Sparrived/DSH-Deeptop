...
- **DSH 运行时问题**：构建矩阵会先从 `vendor/dsh` 子模块生成并校验平台对应的压缩 `dsh-runtime.tar.gz`；应用首次启动时由 Rust 安全解压到按源码提交、平台和架构命名的本地缓存，后续更新复用匹配缓存；运行时只通过系统 Node.js 执行缓存入口，不访问 npm registry，也不回退到 PATH 或全局 npm。
