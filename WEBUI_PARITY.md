...
- 对比版本：`d3eceb61eea73ce1f4e624e346bbec9e857c10eb`（`dsh-v0.1.5-rc.1`，基于官方 `dsh-v0.1.5-rc.1` 并保留 Deeptop 的 preset fork 迁移、pi-ai 思考 tokens、PowerShell Store 重探测、storage-json 原子替换重试、缺失历史附件投影、官方 http-proxy 桌面策略与标题请求思考抑制七条补丁）。
- 运行时版本：`@deepseek-ai/dsh@0.1.5-rc.1`，由 `vendor/dsh` 子模块源码构建。
...
- [x] Host/Cordis 插件：Deeptop Profile 与 DSH Host 共用同一套 Service、Provider、Session 和 ApiProxy；新增插件可以继续通过 Profile 注入。
...
- 生成文件的产出词表对齐 Web 的 `turn-deliverables`：路径取自**变更调用的参数**（`write` 要求 `content` 为字符串、`edit` 要求新旧内容不同、`str_replace_editor` 只认 `create`/`str_replace`/`insert`），失败结果与只读命令不计入。此前只读结果元数据 `meta.diffs`，而新建文件的该字段为空数组，因此新建文件从不出现在生成文件卡片里（PTC 内外都受影响）。与 Web 一致，PTC 子派发不独立产出文件，PTC 交付由程序显式调用 `present` 完成。
